...
SELECT *
FROM FATURAS
WHERE Total_Fatura BETWEEN 10000 AND 30000 AND (Imposto_Fatura / Total_Fatura) BETWEEN 0.20 AND 0.30;
GO
...
SELECT 
    C.Codigo_Cliente AS 'Código do Cliente', 
    C.Nome_Cliente AS 'Nome do Cliente', 
    ISNULL(F.Numero_fatura, 'Nada consta') AS 'Número da Fatura', 
    ISNULL(F.Total_Fatura, 'Nada consta') AS 'Total da Fatura'
FROM CLIENTES AS C
LEFT JOIN FATURAS AS F ON C.Codigo_Cliente = F.Codigo_Cliente
ORDER BY C.Codigo_Cliente, F.Total_Fatura;
GO
...
FROM CLIENTES AS C
JOIN LINHAS AS L ON C.Codigo_Cliente = L.Codigo_Cliente;
GO
...
SELECT 
    ...
    P.Valor_UnitarioProduto AS 'Valor Unitário',
    P.Taxa_Desconto_Produto AS 'Taxa de Desconto',
    P.Valor_UnitarioProduto * (1 - P.Taxa_Desconto_Produto) AS 'Valor com Desconto',
    F.Total_Fatura AS 'Total da Fatura'
FROM CLIENTES AS C
JOIN FATURAS AS F ON C.Codigo_Cliente = F.Codigo_Cliente
JOIN LINHAS AS L ON F.Numero_fatura = L.Numero_Fatura
JOIN PRODUTOS AS P ON L.Codigo_Produto = P.Codigo_Produto
WHERE C.Codigo_Cliente = 1
GROUP BY C.Nome_Cliente, F.Numero_fatura, P.Descricao_Produto, P.Valor_UnitarioProduto, P.Taxa_Desconto_Produto, F.Total_Fatura;
GO
...
SELECT 
    C.Codigo_Cliente AS 'Código do Cliente', 
    C.Nome_Cliente AS 'Nome do Cliente', 
    COUNT(DISTINCT L.Codigo_Produto) AS 'Número Total de Produtos Comprados',
    FORMAT(MIN(F.Total_Fatura), 'C', 'pt-br') AS 'Menor Fatura',
    FORMAT(MAX(F.Total_Fatura), 'C', 'pt-br') AS 'Maior Fatura'
FROM CLIENTES AS C
JOIN FATURAS AS F ON C.Codigo_Cliente = F.Codigo_Cliente
JOIN LINHAS AS L ON F.Numero_fatura = L.Numero_Fatura
...
WITH CTE AS (
    SELECT 
        c.Codigo_Cliente AS 'Codigo do Cliente', 
        c.Nome_Cliente AS 'Nome do Cliente', 
        ISNULL(SUM(l.Quantidade_Produto_Linha), 0) AS 'Quantidade Total de Produtos Comprados'
    FROM 
        CLIENTES c
    LEFT JOIN 
...
SELECT 
    ...
    DENSE_RANK() OVER (ORDER BY 'Quantidade Total de Produtos Comprados' DESC) AS 'Classificacao Final'
FROM 
    CTE
ORDER BY 
    'Quantidade Total de Produtos Comprados' DESC;
GO
...
WITH CTE AS (
    SELECT 
        f.Codigo_Fornecedor AS 'Codigo do Fornecedor', 
        f.Nome_Fornecedor AS 'Nome do Fornecedor', 
        p.Codigo_Produto AS 'Codigo do Produto', 
        p.Descricao_Produto AS 'Descricao do Produto', 
        ISNULL(SUM(l.Quantidade_Produto_Linha), 0) AS 'Quantidade Total Vendida'
    FROM 
        FORNECEDORES f
    LEFT JOIN 
        PRODUTOS p ON f.Codigo_Fornecedor = p.Codigo_Fornecedor
    LEFT JOIN 
        LINHAS l ON p.Codigo_Produto = l.Codigo_Produto
    GROUP BY 
        f.Codigo_Fornecedor, f.Nome_Fornecedor, p.Codigo_Produto, p.Descricao_Produto
)
...
SELECT 
    ...
    DENSE_RANK() OVER (PARTITION BY 'Nome do Fornecedor' ORDER BY 'Quantidade Total Vendida' DESC) AS 'Classificacao Final'
FROM 
    CTE
ORDER BY 
    'Nome do Fornecedor', 'Quantidade Total Vendida' DESC;
GO